...
## Running the application and testing via API

1. Start the databases (and optionally the observability stack):  
   `docker compose up -d`
2. Run the Spring Boot app:  
   `mvn spring-boot:run` (from the project root), or run `PaymentExampleV2Application` from your IDE.
3. The app listens on **http://localhost:8080**. Test endpoints:

| Method | URL                                                                    | Description                                                          |
|--------|------------------------------------------------------------------------|----------------------------------------------------------------------|
| GET    | `/api/health`                                                          | Health check (no DB).                                                |
| GET    | `/api/analytics/overall?startDate=2025-01-01&endDate=2025-12-31`       | Cross-shard overall metrics for the date range.                      |
| GET    | `/api/analytics/user/{userId}?startDate=2025-01-01&endDate=2025-12-31` | Single-shard metrics for one user (e.g. `/api/analytics/user/1001`). |

Date params are optional; use ISO date format `yyyy-MM-dd`.
